...
describe('TodoItem >', () => {
  it(`calls onCompleteChange() when checkbox is clicked`, () => {
    ...
    expect(onCompleteChange.mock.calls[0]).toEqual(['item0', true]);
  });

  it(`calls onDelete() when delete button and delete confirm button are clicked`, () => {
    const onDelete = jest.fn();
    const itemEl = render(<TodoItem id="item0" onDelete={onDelete} />);
    ...
    expect(onDelete.mock.calls[0]).toEqual(['item0']);
  });
